...
/*
 *
 * Originally written from the Arjuna system, but we may as well also
 * use a cut down version for C++SIM - helps to keep things consistent.
 *
 ...
 *    _cppsim_debug must be set to the level of tracing required.
...
#define debug_stream ((_cppsim_debug != 0)?(_cppsim_debug->stream()):(_cppsim_debug = new DebugController(),_cppsim_debug->stream()))
